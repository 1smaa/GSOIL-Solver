...
def main() -> None:
    ...
    plt.plot(wavelengths*1e9, np.abs(E_fft_prim_shifted), label="Primary")
    plt.plot(wavelengths*1e9, np.abs(E_fft_sec_shifted), label="Secondary")
    plt.xlabel("Wavelength (nm)")
    plt.ylabel("Amplitude (a.u.)")
    plt.title("Optical Spectrum Centered at 1560 nm")
    plt.legend()
    plt.minorticks_on()
    plt.grid(True, which='both', linestyle='--', alpha=0.5)
    plt.xlim(1555,1566)
    plt.xticks(np.arange(1555, 1566, 0.2)) # zoom around carrier
    ax = plt.gca()
