...
class StandardPropagation(object):
    ...
    def step(self) -> None:
        ...
        phi_new=(self._phi+self._dt*self._params["alpha"]*0.5*(self._params["g"]*(self._n-self._params["N_TR"])))#%(2*np.pi)
        self._n=max(n_new,0)
        self._s=max(s_new,1)
        self._phi=phi_new
    ...
    def _langevin_noise(self) -> None:
        def F_S() -> float:
            coeff=(2*self._params["beta"]*self._n*self._s)/(self._params["tau_p"]*self._dt)
            if coeff<0: return 0
            return np.sqrt(coeff)*np.random.standard_normal(1)
        def F_PHI() -> float:
            coeff=(self._params["beta"]*self._n)/(2*self._params["tau_p"]*self._s*self._dt)
            if coeff<0: return 0
            return np.sqrt(coeff)*np.random.standard_normal(1)
        def F_Z() -> float:
            coeff=(2*self._n)/(self._params["V"]*self._params["tau_n"]*self._dt)
            if coeff<0: return 0
            return np.sqrt(coeff)*np.random.standard_normal(1)
...
class SecondaryPropagation(StandardPropagation):
    ...
    def step(self) -> None:
        ...
        if not np.isnan(master_n):
            print(f"Master parameters: {master_n},{master_s},{master_phi}")
            print(f"Slave parameters: {self._n},{self._s},{self._phi}")
        ...
        dphi_dt_free=self._params["alpha"]*0.5*(self._params["g"]*(self._n-self._params["N_TR"]))
        ...
        phi_new=(self._phi+self._dt*dphi_dt)#%(2*np.pi)
        self._n=n_new
        self._s=s_new
        self._phi=phi_new
